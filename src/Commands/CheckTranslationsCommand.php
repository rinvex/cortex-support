...
#[AsCommand(name: 'cortex:check-translations')]
class CheckTranslationsCommand extends Command implements PromptsForMissingInput
{
    ...
    protected $name = 'cortex:check-translations';
    ...
    public function handle(): int
    {
        ...
        $this->scan('cortex/panels');
        $this->scan('cortex/actions');
        $this->scan('cortex/forms');
        $this->scan('cortex/infolists');
        $this->scan('cortex/notifications');
        $this->scan('cortex/schemas');
        $this->scan('cortex/support');
        $this->scan('cortex/tables');
        $this->scan('cortex/widgets');
    ...
    protected function scan(string $package): void
    {
        $localeRootDirectory = match ($source = $this->option('source')) {
            'resources' => lang_path("vendor/{$package}"),
            'app' => app_path("{$package}/resources/lang"),
        ...
        collect($filesystem->directories($localeRootDirectory))
            ...
            ->each(function (string $locale, string $localeDir) use ($filesystem, $localeRootDirectory, $package): void {
                ...
                if ($missingFiles->count() > 0 && $removedFiles->count() > 0) {
                    warning("[!] Package {$package} has {$missingFiles->count()} missing translation " . Str::plural('file', $missingFiles->count()) . " and {$removedFiles->count()} removed translation " . Str::plural('file', $missingFiles->count()) . ' for ' . locale_get_display_name($locale, 'en') . ".\n");
                } elseif ($missingFiles->count() > 0) {
                    warning("[!] Package {$package} has {$missingFiles->count()} missing translation " . Str::plural('file', $missingFiles->count()) . ' for ' . locale_get_display_name($locale, 'en') . ".\n");
                } elseif ($removedFiles->count() > 0) {
                    warning("[!] Package {$package} has {$removedFiles->count()} removed translation " . Str::plural('file', $removedFiles->count()) . ' for ' . locale_get_display_name($locale, 'en') . ".\n");
                }
                ...
                collect($files)
                    ...
                    ->tap(function (Collection $files) use ($locale, $package): void {
                        ...
                        if ((! $missingKeysCount) && (! $removedKeysCount)) {
                            info("[✓] Package {$package} has no missing or removed translation keys for {$locale}!\n");
                        } elseif ($missingKeysCount && $removedKeysCount) {
                            warning("[!] Package {$package} has {$missingKeysCount} missing translation " . Str::plural('key', $missingKeysCount) . " and {$removedKeysCount} removed translation " . Str::plural('key', $removedKeysCount) . " for {$locale}.\n");
                        } elseif ($missingKeysCount) {
                            warning("[!] Package {$package} has {$missingKeysCount} missing translation " . Str::plural('key', $missingKeysCount) . " for {$locale}.\n");
                        } elseif ($removedKeysCount) {
                            warning("[!] Package {$package} has {$removedKeysCount} removed translation " . Str::plural('key', $removedKeysCount) . " for {$locale}.\n");
                        }
                    })
