...
use Cortex\Support\Contracts\ScalableIcon;
use Illuminate\Contracts\Support\Htmlable;
use Illuminate\View\ComponentAttributeBag;
use Illuminate\Translation\MessageSelector;
use Cortex\Support\View\Components\Contracts\HasColor;

if (! function_exists('Cortex\Support\get_model_label')) {
    /**
...
if (! function_exists('Cortex\Support\locale_has_pluralization')) {
...
if (! function_exists('Cortex\Support\get_component_color_classes')) {
    /**
    ...
    function get_component_color_classes(string | HasColor $component, ?string $color): array
    {
        ...
        return CortexColor::getComponentClasses($component, $color);
    }
}

if (! function_exists('Cortex\Support\get_color_css_variables')) {
    /**
    ...
    function get_color_css_variables(string | array | null $color, array $shades, ?string $alias = null): ?string
    {
        ...
        if ($alias !== null) {
            if (($overridingShades = CortexColor::getOverridingShades($alias)) !== null) {
                $shades = $overridingShades;
            }

            if ($addedShades = CortexColor::getAddedShades($alias)) {
                $shades = [...$shades, ...$addedShades];
            }

            if ($removedShades = CortexColor::getRemovedShades($alias)) {
...
if (! function_exists('Cortex\Support\prepare_inherited_attributes')) {
...
if (! function_exists('Cortex\Support\is_slot_empty')) {
...
if (! function_exists('Cortex\Support\is_app_url')) {
...
if (! function_exists('Cortex\Support\generate_href_html')) {
    function generate_href_html(?string $url, bool $shouldOpenInNewTab = false, ?bool $shouldOpenInSpaMode = null): Htmlable
    {
        ...
        if ($shouldOpenInNewTab) {
            $html .= ' target="_blank"';
        } elseif ($shouldOpenInSpaMode ?? (cortex()->getCurrentOrDefaultPanel()->hasSpaMode($url))) {
            $html .= ' wire:navigate';
        }
...
if (! function_exists('Cortex\Support\generate_icon_html')) {
    ...
    {
        if (filled($alias)) {
            $icon = CortexIcon::resolve($alias) ?: $icon;
        }
...
if (! function_exists('Cortex\Support\generate_loading_indicator_html')) {
...
if (! function_exists('Cortex\Support\generate_search_column_expression')) {
    /**
     * @internal This function is only to be used internally by Cortex and is subject to change at any time. Please do not use this function in your own code.
     */
...
if (! function_exists('Cortex\Support\generate_search_term_expression')) {
    /**
     * @internal This function is only to be used internally by Cortex and is subject to change at any time. Please do not use this function in your own code.
     */
...
if (! function_exists('Cortex\Support\original_request')) {
