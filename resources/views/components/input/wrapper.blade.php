...
@php
    use Cortex\Support\View\Components\InputComponent\WrapperComponent\IconComponent;

    $prefixActions = array_filter(
        $prefixActions,
        fn (\Cortex\Actions\Action $prefixAction): bool => $prefixAction->isVisible(),
    );

    $suffixActions = array_filter(
        $suffixActions,
        fn (\Cortex\Actions\Action $suffixAction): bool => $suffixAction->isVisible(),
    );
    ...
    $getIconClasses = fn (string $color = 'secondary'): string => \Illuminate\Support\Arr::toCssClasses([
        ...\Cortex\Support\get_component_color_classes(IconComponent::class, $color),
    ]);
...
>
    @if ($hasPrefix || $hasLoadingIndicator)
        <div
            @if (! $hasPrefix)
                wire:loading.delay.{{ config('cortex.support.livewire_loading_delay', 'default') }}.flex
        ...
        >
            ...
            {{
                \Cortex\Support\generate_icon_html($prefixIcon, $prefixIconAlias, (new \Illuminate\View\ComponentAttributeBag)
                    ->merge([
            ...
            @if ($hasLoadingIndicator)
                {{
                    \Cortex\Support\generate_loading_indicator_html((new \Illuminate\View\ComponentAttributeBag([
                        'wire:loading.delay.' . config('cortex.support.livewire_loading_delay', 'default') => $hasPrefix,
    ...
    <div
        @if ($hasLoadingIndicator && (! $hasPrefix))
            @if ($inlinePrefix)
                wire:loading.delay.{{ config('cortex.support.livewire_loading_delay', 'default') }}.class.remove="ps-3"
            @endif
    ...
    @if ($hasSuffix)
        ...
        >
            ...
            {{
                \Cortex\Support\generate_icon_html($suffixIcon, $suffixIconAlias, (new \Illuminate\View\ComponentAttributeBag)
                    ->merge([
                        'wire:loading.remove.delay.' . config('cortex.support.livewire_loading_delay', 'default') => $hasLoadingIndicator,
                        'wire:target' => $hasLoadingIndicator ? $loadingIndicatorTarget : false,
                    ], escape: false)
